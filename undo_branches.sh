#!/bin/sh -e
# Generated by patch2vi from unified diff
# Available separators: 0001-0010 0013-0014 0016-0037 0102-0104 0107-0110 0112 0120-0121 0124 0126-0132 0136 0176-0377

# Pass any argument to use patch(1) instead of nextvi ex commands
if [ -n "$1" ]; then
    sed '1,/^=== PATCH2VI PATCH ===$/d' "$0" | patch -p1 --merge=diff3
    exit $?
fi

VI=${VI:-vi}
if ! $VI -? 2>&1 | grep -q 'Nextvi'; then
    echo "Error: $VI is not nextvi" >&2
    echo "Set VI environment variable to point to nextvi binary" >&2
    exit 1
fi

SEP="$(printf '\001')"
# Comment to continue despite errors (errors are still printed)
QF="\\${SEP}vis 2\\${SEP}q!1"
# Uncomment to enter interactive vi on patch failure
#INTR="\\${SEP}|sc|\\${SEP}vis 2:e $0:%f>:@Q:q!1"
# Uncomment to skip errors (0? = silent nop)
#DBG="0\?"

# Patch: ex.c
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}%;f> 
static void \\\\*ec_null\\\\(char \\\\*loc, char \\\\*cmd, char \\\\*arg\\\\) \\\\{ return NULL; \\\\}

${SEP}??!${DBG:-re p FAIL line 1421\\${SEP}p FAIL line 1421${INTR}${QF}}${SEP};=
${SEP}+2a static void *ec_undoleafs(char *loc, char *cmd, char *arg)
{
	char *s = lbuf_getleafs(xb);
	if (*arg)
		lbuf_setleaf(xb, atoi(arg));
	else
		ex_print(s, msg_ft)
	return NULL;
}

.
${SEP}.,\$;f> 	\\\\{\"uc\", ec_setenc\\\\},
	\\\\{\"uz\", ec_setenc\\\\},
	\\\\{\"ub\", ec_setenc\\\\},${SEP}??!${DBG:-re p FAIL line 1516\\${SEP}p FAIL line 1516${INTR}${QF}}${SEP};=
${SEP}+2a 	{\"up\", ec_undoleafs},
.
${SEP}vis 2${SEP}wq" $VI -e 'ex.c'

# Patch: lbuf.c
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}%;f> 	struct lbuf \\\\*lb = emalloc\\\\(sizeof\\\\(\\\\*lb\\\\)\\\\);
	memset\\\\(lb, 0, sizeof\\\\(\\\\*lb\\\\)\\\\);
	memset\\\\(lb->mark, -1, sizeof\\\\(lb->mark\\\\) / 2\\\\);${SEP}??!${DBG:-re p FAIL line 5\\${SEP}p FAIL line 5${INTR}${QF}}${SEP};=
${SEP}+2a 	sbuf_make(lb->rehist, 128)
.
${SEP}.,\$;f> 	return 0;
\\\\}

${SEP}??!${DBG:-re p FAIL line 61\\${SEP}p FAIL line 61${INTR}${QF}}${SEP};=
${SEP}+2a static void lbuf_freeleafs(struct lbuf *lb)
{
	char *ptr = lb->rehist->s;
	struct lopt *hist;
	int n, i, c;
	sbuf *freedsb; sbuf_make(freedsb, 128)
	for (i = 0; i < lb->hist_n; i++) {
		sbuf_mem(freedsb, &lb->hist[i].mark, (int)sizeof(void*))
		lopt_done(&lb->hist[i]);
	}
	while (lb->rehist->s_n > ptr - lb->rehist->s) {
		n = *(int*)ptr;
		hist = (struct lopt*)&ptr[sizeof(n)];
		for (i = 0; i < n; i++) {
			for (c = 0; c < freedsb->s_n; c+=sizeof(void*))
				if (*(void**)((char*)freedsb->s + c) == hist[i].mark)
					goto skip;
			sbuf_mem(freedsb, &hist[i].mark, (int)sizeof(void*))
			lopt_done(&hist[i]);
			skip:;
		}
		ptr += n * sizeof(lb->hist[0]) + sizeof(n);
	}
	sbuf_free(lb->rehist)
	sbuf_free(freedsb)
}

.
${SEP}.,\$;f> 	int i;
	for \\\\(i = 0; i < lb->ln_n; i\\\\+\\\\+\\\\)
		free\\\\(lbuf_i\\\\(lb, i\\\\)\\\\);${SEP}??!${DBG:-re p FAIL line 67\\${SEP}p FAIL line 67${INTR}${QF}}${SEP};=
${SEP}+3,#+1c 	lbuf_freeleafs(lb);
.
${SEP}.,\$;f> 		lopt_done\\\\(lo\\\\);
\\\\}

${SEP}??!${DBG:-re p FAIL line 141\\${SEP}p FAIL line 141${INTR}${QF}}${SEP};=
${SEP}+2a void lbuf_setleaf(struct lbuf *lb, int leaf)
{
	char *ptr1 = lb->rehist->s, *ptr2;
	int n, i, off = -1;
	for (i = 0; lb->rehist->s_n > ptr1 - lb->rehist->s; i++) {
		n = *(int*)ptr1;
		if (i == leaf) {
			off = ptr1 - lb->rehist->s;
			break;
		}
		ptr1 += n * sizeof(lb->hist[0]) + sizeof(lb->hist_n);
	}
	if (off < 0)
		return;
	while (!lbuf_undo(lb, &xrow, &xoff));
	i = lb->hist_n - lb->hist_u;
	if (i) {
		sbuf_mem(lb->rehist, &lb->hist_n, (int)sizeof(lb->hist_n))
		sbuf_mem(lb->rehist, lb->hist, (int)(lb->hist_n * sizeof(lb->hist[0])));
		ptr1 = lb->rehist->s + off;
		lb->hist_n = n;
		lb->hist_u = 0;
		if (lb->hist_n >= lb->hist_sz) {
			free(lb->hist);
			int sz = lb->hist_n * 2;
			struct lopt *hist = emalloc(sz * sizeof(hist[0]));
			lb->hist = hist;
			lb->hist_sz = sz;
		}
		memcpy(lb->hist, &ptr1[sizeof(n)], n * sizeof(lb->hist[0]));
		ptr2 = &ptr1[n * sizeof(lb->hist[0]) + sizeof(n)];
		if (lb->rehist->s_n > ptr2 - lb->rehist->s)
			memmove(ptr1, ptr2, lb->rehist->s_n - (ptr2 - lb->rehist->s));
		lb->rehist->s_n -= ptr2 - ptr1;
		while (!lbuf_redo(lb, &xrow, &xoff));
	}
}

char *lbuf_getleafs(struct lbuf *lb)
{
	char *ptr = lb->rehist->s, *bptr;
	int n, i;
	char buf[100];
	sbuf_smake(sb, 128)
	for (i = 0; lb->rehist->s_n > ptr - lb->rehist->s; i++) {
		n = *(int*)ptr;
		bptr = itoa(i, buf);
		*bptr++ = '=';
		bptr = itoa(n, bptr);
		*bptr++ = '|';
		*bptr++ = '\\\\0';
		sbuf_str(sb, buf);
		ptr += n * sizeof(lb->hist[0]) + sizeof(lb->hist_n);
	}
	sbufn_ret(sb, sb->s)
}

.
${SEP}.,\$;f> 	if \\\\(xseq < 0\\\\)
		lo = &slo;
	else \\\\{${SEP}??!${DBG:-re p FAIL line 150\\${SEP}p FAIL line 150${INTR}${QF}}${SEP};=
${SEP}+3,#+1c 		int i = lb->hist_n - lb->hist_u;
		if (i) {
			sbuf_mem(lb->rehist, &lb->hist_n, (int)sizeof(lb->hist_n))
			sbuf_mem(lb->rehist, lb->hist, (int)(lb->hist_n * sizeof(lb->hist[0])));
		}
.
${SEP}.,\$;f> void lbuf_saved\\\\(struct lbuf \\\\*lb, int clear\\\\)
\\\\{
	if \\\\(clear\\\\) \\\\{${SEP}??!${DBG:-re p FAIL line 438\\${SEP}p FAIL line 438${INTR}${QF}}${SEP};=
${SEP}+3,#+1c 		lbuf_freeleafs(lb);
.
${SEP}.,\$;f> 		lb->hist_n = 0;
		lb->hist_u = 0;${SEP}??!${DBG:-re p FAIL line 441\\${SEP}p FAIL line 441${INTR}${QF}}${SEP};=
${SEP}+1a 		sbuf_make(lb->rehist, 128)
.
${SEP}vis 2${SEP}wq" $VI -e 'lbuf.c'

# Patch: vi.h
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}%;f> struct lbuf \\\\{
	char \\\\*\\\\*ln;			/\\\\* buffer lines \\\\*/
	struct lopt \\\\*hist;		/\\\\* buffer history \\\\*/${SEP}??!${DBG:-re p FAIL line 139\\${SEP}p FAIL line 139${INTR}${QF}}${SEP};=
${SEP}+2a 	sbuf *rehist;		/* alternate redo timelines */
.
${SEP}.,\$;f> int lbuf_findchar\\\\(struct lbuf \\\\*lb, char \\\\*cs, int cmd, int n, int \\\\*r, int \\\\*o\\\\);
int lbuf_search\\\\(struct lbuf \\\\*lb, rset \\\\*re, int dir, int beg, int end, int pskip,
		int nskip, int \\\\*r, int \\\\*o\\\\);${SEP}??!${DBG:-re p FAIL line 178\\${SEP}p FAIL line 178${INTR}${QF}}${SEP};=
${SEP}+2a void lbuf_setleaf(struct lbuf *lb, int leaf);
char *lbuf_getleafs(struct lbuf *lb);
.
${SEP}vis 2${SEP}wq" $VI -e 'vi.h'

exit 0
=== PATCH2VI DELTA ===
=== PATCH2VI PATCH ===
diff --git a/ex.c b/ex.c
index 36b8a6d6..2c136687 100644
--- a/ex.c
+++ b/ex.c
@@ -1419,6 +1419,16 @@ static void *ec_krsset(char *loc, char *cmd, char *arg)
 
 static void *ec_null(char *loc, char *cmd, char *arg) { return NULL; }
 
+static void *ec_undoleafs(char *loc, char *cmd, char *arg)
+{
+	char *s = lbuf_getleafs(xb);
+	if (*arg)
+		lbuf_setleaf(xb, atoi(arg));
+	else
+		ex_print(s, msg_ft)
+	return NULL;
+}
+
 static int eo_val(char *arg)
 {
 	int val = atoi(arg);
@@ -1514,6 +1524,7 @@ static struct excmd {
 	{"uc", ec_setenc},
 	{"uz", ec_setenc},
 	{"ub", ec_setenc},
+	{"up", ec_undoleafs},
 	{"ud", ec_undoredo},
 	EO(shape),
 	EO(seq),
diff --git a/lbuf.c b/lbuf.c
index 09950361..3be397fc 100644
--- a/lbuf.c
+++ b/lbuf.c
@@ -3,6 +3,7 @@ struct lbuf *lbuf_make(void)
 	struct lbuf *lb = emalloc(sizeof(*lb));
 	memset(lb, 0, sizeof(*lb));
 	memset(lb->mark, -1, sizeof(lb->mark) / 2);
+	sbuf_make(lb->rehist, 128)
 	return lb;
 }
 
@@ -59,13 +60,39 @@ int lbuf_jump(struct lbuf *lb, int mark, int *pos, int *off)
 	return 0;
 }
 
+static void lbuf_freeleafs(struct lbuf *lb)
+{
+	char *ptr = lb->rehist->s;
+	struct lopt *hist;
+	int n, i, c;
+	sbuf *freedsb; sbuf_make(freedsb, 128)
+	for (i = 0; i < lb->hist_n; i++) {
+		sbuf_mem(freedsb, &lb->hist[i].mark, (int)sizeof(void*))
+		lopt_done(&lb->hist[i]);
+	}
+	while (lb->rehist->s_n > ptr - lb->rehist->s) {
+		n = *(int*)ptr;
+		hist = (struct lopt*)&ptr[sizeof(n)];
+		for (i = 0; i < n; i++) {
+			for (c = 0; c < freedsb->s_n; c+=sizeof(void*))
+				if (*(void**)((char*)freedsb->s + c) == hist[i].mark)
+					goto skip;
+			sbuf_mem(freedsb, &hist[i].mark, (int)sizeof(void*))
+			lopt_done(&hist[i]);
+			skip:;
+		}
+		ptr += n * sizeof(lb->hist[0]) + sizeof(n);
+	}
+	sbuf_free(lb->rehist)
+	sbuf_free(freedsb)
+}
+
 void lbuf_free(struct lbuf *lb)
 {
 	int i;
 	for (i = 0; i < lb->ln_n; i++)
 		free(lbuf_i(lb, i));
-	for (i = 0; i < lb->hist_n; i++)
-		lopt_done(&lb->hist[i]);
+	lbuf_freeleafs(lb);
 	free(lb->hist);
 	free(lb->ln);
 	free(lb);
@@ -139,6 +166,63 @@ void lbuf_emark(struct lbuf *lb, struct lopt *lo, int end, int o2)
 		lopt_done(lo);
 }
 
+void lbuf_setleaf(struct lbuf *lb, int leaf)
+{
+	char *ptr1 = lb->rehist->s, *ptr2;
+	int n, i, off = -1;
+	for (i = 0; lb->rehist->s_n > ptr1 - lb->rehist->s; i++) {
+		n = *(int*)ptr1;
+		if (i == leaf) {
+			off = ptr1 - lb->rehist->s;
+			break;
+		}
+		ptr1 += n * sizeof(lb->hist[0]) + sizeof(lb->hist_n);
+	}
+	if (off < 0)
+		return;
+	while (!lbuf_undo(lb, &xrow, &xoff));
+	i = lb->hist_n - lb->hist_u;
+	if (i) {
+		sbuf_mem(lb->rehist, &lb->hist_n, (int)sizeof(lb->hist_n))
+		sbuf_mem(lb->rehist, lb->hist, (int)(lb->hist_n * sizeof(lb->hist[0])));
+		ptr1 = lb->rehist->s + off;
+		lb->hist_n = n;
+		lb->hist_u = 0;
+		if (lb->hist_n >= lb->hist_sz) {
+			free(lb->hist);
+			int sz = lb->hist_n * 2;
+			struct lopt *hist = emalloc(sz * sizeof(hist[0]));
+			lb->hist = hist;
+			lb->hist_sz = sz;
+		}
+		memcpy(lb->hist, &ptr1[sizeof(n)], n * sizeof(lb->hist[0]));
+		ptr2 = &ptr1[n * sizeof(lb->hist[0]) + sizeof(n)];
+		if (lb->rehist->s_n > ptr2 - lb->rehist->s)
+			memmove(ptr1, ptr2, lb->rehist->s_n - (ptr2 - lb->rehist->s));
+		lb->rehist->s_n -= ptr2 - ptr1;
+		while (!lbuf_redo(lb, &xrow, &xoff));
+	}
+}
+
+char *lbuf_getleafs(struct lbuf *lb)
+{
+	char *ptr = lb->rehist->s, *bptr;
+	int n, i;
+	char buf[100];
+	sbuf_smake(sb, 128)
+	for (i = 0; lb->rehist->s_n > ptr - lb->rehist->s; i++) {
+		n = *(int*)ptr;
+		bptr = itoa(i, buf);
+		*bptr++ = '=';
+		bptr = itoa(n, bptr);
+		*bptr++ = '|';
+		*bptr++ = '\0';
+		sbuf_str(sb, buf);
+		ptr += n * sizeof(lb->hist[0]) + sizeof(lb->hist_n);
+	}
+	sbufn_ret(sb, sb->s)
+}
+
 /* append undo/redo history */
 struct lopt *lbuf_opt(struct lbuf *lb, int beg, int o1, int n_del)
 {
@@ -147,8 +231,11 @@ struct lopt *lbuf_opt(struct lbuf *lb, int beg, int o1, int n_del)
 	if (xseq < 0)
 		lo = &slo;
 	else {
-		for (int i = lb->hist_u; i < lb->hist_n; i++)
-			lopt_done(&lb->hist[i]);
+		int i = lb->hist_n - lb->hist_u;
+		if (i) {
+			sbuf_mem(lb->rehist, &lb->hist_n, (int)sizeof(lb->hist_n))
+			sbuf_mem(lb->rehist, lb->hist, (int)(lb->hist_n * sizeof(lb->hist[0])));
+		}
 		lb->hist_n = lb->hist_u;
 		if (lb->hist_n == lb->hist_sz) {
 			int sz = lb->hist_sz + (lb->hist_sz ? lb->hist_sz : 128);
@@ -435,10 +522,10 @@ int lbuf_redo(struct lbuf *lb, int *row, int *off)
 void lbuf_saved(struct lbuf *lb, int clear)
 {
 	if (clear) {
-		for (int i = 0; i < lb->hist_n; i++)
-			lopt_done(&lb->hist[i]);
+		lbuf_freeleafs(lb);
 		lb->hist_n = 0;
 		lb->hist_u = 0;
+		sbuf_make(lb->rehist, 128)
 	}
 	lb->modified = 0;
 	lb->saved = lb->hist_u;
diff --git a/vi.h b/vi.h
index bd944301..099f9be8 100644
--- a/vi.h
+++ b/vi.h
@@ -137,6 +137,7 @@ struct linfo {
 struct lbuf {
 	char **ln;			/* buffer lines */
 	struct lopt *hist;		/* buffer history */
+	sbuf *rehist;		/* alternate redo timelines */
 	int mark[NMARKS * 2];		/* mark rows & offs */
 	int tmp_mark[4];		/* aux mark state */
 	int ln_n;			/* number of lines in ln[] */
@@ -176,6 +177,8 @@ int lbuf_next(struct lbuf *lb, int dir, int *r, int *o);
 int lbuf_findchar(struct lbuf *lb, char *cs, int cmd, int n, int *r, int *o);
 int lbuf_search(struct lbuf *lb, rset *re, int dir, int beg, int end, int pskip,
 		int nskip, int *r, int *o);
+void lbuf_setleaf(struct lbuf *lb, int leaf);
+char *lbuf_getleafs(struct lbuf *lb);
 #define lbuf_dedup(lb, str, n) \
 { for (int i = 0; i < lbuf_len(lb);) { \
 	char *s = lbuf_get(lb, i); \
