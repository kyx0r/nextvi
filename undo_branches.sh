#!/bin/sh
# Generated by patch2vi from unified diff
# Available separators: 
set -e

# Path to nextvi (adjust as needed)
VI=${VI:-vi}

# Verify that VI is nextvi
if ! $VI -? 2>&1 | grep -q 'Nextvi'; then
    echo "Error: $VI is not nextvi" >&2
    echo "Set VI environment variable to point to nextvi" >&2
    exit 1
fi

# Patch: ex.c
SEP="$(printf '\x01')"
EXINIT="rcm:|sc! \\\\${SEP}|vis 6${SEP}%;f> 
static void \\\\*ec_null\\\\(char \\\\*loc, char \\\\*cmd, char \\\\*arg\\\\) \\\\{ return NULL; \\\\}
${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1326\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a static void *ec_undoleafs(char *loc, char *cmd, char *arg)
{
	char *s = lbuf_getleafs(xb);
	if (*arg)
		lbuf_setleaf(xb, atoi(arg));
	else
		ex_print(s, msg_ft)
	return NULL;
}

.
${SEP}.,$;f+ 	\\\\{\"uc\", ec_setenc\\\\},
	\\\\{\"uz\", ec_setenc\\\\},
	\\\\{\"ub\", ec_setenc\\\\},${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1417\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a 	{\"up\", ec_undoleafs},
.
${SEP}vis 4${SEP}wq" $VI -e 'ex.c'

# Patch: lbuf.c
SEP="$(printf '\x01')"
EXINIT="rcm:|sc! \\\\${SEP}|vis 6${SEP}%;f> 	struct lbuf \\\\*lb = emalloc\\\\(sizeof\\\\(\\\\*lb\\\\)\\\\);
	memset\\\\(lb, 0, sizeof\\\\(\\\\*lb\\\\)\\\\);
	memset\\\\(lb->mark, -1, sizeof\\\\(lb->mark\\\\) / 2\\\\);${SEP}??!.-5,.+5p\\${SEP}p FAIL line 5\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a 	sbuf_make(lb->rehist, 128)
.
${SEP}.,$;f+ 	return 0;
\\\\}
${SEP}??!.-5,.+5p\\${SEP}p FAIL line 61\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a static void lbuf_freeleafs(struct lbuf *lb)
{
	char *ptr = lb->rehist->s;
	struct lopt *hist;
	int n, i, c;
	sbuf *freedsb; sbuf_make(freedsb, 128)
	for (i = 0; i < lb->hist_n; i++) {
		sbuf_mem(freedsb, &lb->hist[i].mark, (int)sizeof(void*))
		lopt_done(&lb->hist[i]);
	}
	while (lb->rehist->s_n > ptr - lb->rehist->s) {
		n = *(int*)ptr;
		hist = (struct lopt*)&ptr[sizeof(n)];
		for (i = 0; i < n; i++) {
			for (c = 0; c < freedsb->s_n; c+=sizeof(void*))
				if (*(void**)((char*)freedsb->s + c) == hist[i].mark)
					goto skip;
			sbuf_mem(freedsb, &hist[i].mark, (int)sizeof(void*))
			lopt_done(&hist[i]);
			skip:;
		}
		ptr += n * sizeof(lb->hist[0]) + sizeof(n);
	}
	sbuf_free(lb->rehist)
	sbuf_free(freedsb)
}

.
${SEP}.,$;f+ 	int i;
	for \\\\(i = 0; i < lb->ln_n; i\\\\+\\\\+\\\\)
		free\\\\(lbuf_i\\\\(lb, i\\\\)\\\\);${SEP}??!.-5,.+5p\\${SEP}p FAIL line 67\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+3,#+1c 	lbuf_freeleafs(lb);
.
${SEP}.,$;f+ 		lopt_done\\\\(lo\\\\);
\\\\}
${SEP}??!.-5,.+5p\\${SEP}p FAIL line 144\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a void lbuf_setleaf(struct lbuf *lb, int leaf)
{
	char *ptr1 = lb->rehist->s, *ptr2;
	int n, i, off = -1;
	for (i = 0; lb->rehist->s_n > ptr1 - lb->rehist->s; i++) {
		n = *(int*)ptr1;
		if (i == leaf) {
			off = ptr1 - lb->rehist->s;
			break;
		}
		ptr1 += n * sizeof(lb->hist[0]) + sizeof(lb->hist_n);
	}
	if (off < 0)
		return;
	while (!lbuf_undo(lb, &xrow, &xoff));
	i = lb->hist_n - lb->hist_u;
	if (i) {
		sbuf_mem(lb->rehist, &lb->hist_n, (int)sizeof(lb->hist_n))
		sbuf_mem(lb->rehist, lb->hist, (int)(lb->hist_n * sizeof(lb->hist[0])));
		ptr1 = lb->rehist->s + off;
		lb->hist_n = n;
		lb->hist_u = 0;
		if (lb->hist_n >= lb->hist_sz) {
			free(lb->hist);
			int sz = lb->hist_n * 2;
			struct lopt *hist = emalloc(sz * sizeof(hist[0]));
			lb->hist = hist;
			lb->hist_sz = sz;
		}
		memcpy(lb->hist, &ptr1[sizeof(n)], n * sizeof(lb->hist[0]));
		ptr2 = &ptr1[n * sizeof(lb->hist[0]) + sizeof(n)];
		if (lb->rehist->s_n > ptr2 - lb->rehist->s)
			memmove(ptr1, ptr2, lb->rehist->s_n - (ptr2 - lb->rehist->s));
		lb->rehist->s_n -= ptr2 - ptr1;
		while (!lbuf_redo(lb, &xrow, &xoff));
	}
}

char *lbuf_getleafs(struct lbuf *lb)
{
	char *ptr = lb->rehist->s, *bptr;
	int n, i;
	char buf[100];
	sbuf_smake(sb, 128)
	for (i = 0; lb->rehist->s_n > ptr - lb->rehist->s; i++) {
		n = *(int*)ptr;
		bptr = itoa(i, buf);
		*bptr++ = '=';
		bptr = itoa(n, bptr);
		*bptr++ = '|';
		*bptr++ = '\\\\0';
		sbuf_str(sb, buf);
		ptr += n * sizeof(lb->hist[0]) + sizeof(lb->hist_n);
	}
	sbufn_ret(sb, sb->s)
}

.
${SEP}.,$;f+ 	if \\\\(xseq < 0\\\\)
		lo = &slo;
	else \\\\{${SEP}??!.-5,.+5p\\${SEP}p FAIL line 153\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+3,#+1c 		int i = lb->hist_n - lb->hist_u;
		if (i) {
			sbuf_mem(lb->rehist, &lb->hist_n, (int)sizeof(lb->hist_n))
			sbuf_mem(lb->rehist, lb->hist, (int)(lb->hist_n * sizeof(lb->hist[0])));
		}
.
${SEP}.,$;f+ void lbuf_saved\\\\(struct lbuf \\\\*lb, int clear\\\\)
\\\\{
	if \\\\(clear\\\\) \\\\{${SEP}??!.-5,.+5p\\${SEP}p FAIL line 441\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+3,#+1c 		lbuf_freeleafs(lb);
.
${SEP}.,$;f+ 		lb->hist_n = 0;
		lb->hist_u = 0;${SEP}??!.-5,.+5p\\${SEP}p FAIL line 444\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+1a 		sbuf_make(lb->rehist, 128)
.
${SEP}vis 4${SEP}wq" $VI -e 'lbuf.c'

# Patch: vi.h
SEP="$(printf '\x01')"
EXINIT="rcm:|sc! \\\\${SEP}|vis 6${SEP}%;f> struct lbuf \\\\{
	char \\\\*\\\\*ln;			/\\\\* buffer lines \\\\*/
	struct lopt \\\\*hist;		/\\\\* buffer history \\\\*/${SEP}??!.-5,.+5p\\${SEP}p FAIL line 152\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a 	sbuf *rehist;		/* alternate redo timelines */
.
${SEP}.,$;f+ int lbuf_findchar\\\\(struct lbuf \\\\*lb, char \\\\*cs, int cmd, int n, int \\\\*r, int \\\\*o\\\\);
int lbuf_search\\\\(struct lbuf \\\\*lb, rset \\\\*re, int dir, int beg, int end, int pskip,
		int nskip, int \\\\*r, int \\\\*o\\\\);${SEP}??!.-5,.+5p\\${SEP}p FAIL line 191\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a void lbuf_setleaf(struct lbuf *lb, int leaf);
char *lbuf_getleafs(struct lbuf *lb);
.
${SEP}vis 4${SEP}wq" $VI -e 'vi.h'
