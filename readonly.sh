#!/bin/sh
# Generated by patch2vi from unified diff
# Separator: '@'
set -e

# Path to nextvi (adjust as needed)
VI=${VI:-vi}

# Verify that VI is nextvi
if ! $VI -? 2>&1 | grep -q 'Nextvi'; then
    echo "Error: $VI is not nextvi" >&2
    echo "Set VI environment variable to point to nextvi" >&2
    exit 1
fi

# Patch: ex.c
EXINIT="rcm:|sc! @|vis 6@1379a 	{\"ro\", ec_readonly},
.
@vis 4@vis 6@1299a static void *ec_readonly(char *loc, char *cmd, char *arg)
{
	ex_buf->readonly = !ex_buf->readonly;
	return NULL;
}

.
@vis 4@vis 6@660a 			if (ex_buf->readonly)
				return \"write failed: readonly option is set\";
.
@vis 4@vis 6@368a 	if (access(arg, F_OK) == 0 && access(arg, W_OK) == -1)
		ex_buf->readonly = 1;
.
@vis 4@vis 6@115a 	bufs[i].readonly = readonly;
.
@vis 4@vis 6@54a char readonly = 0;		/* commandline readonly option */
.
@vis 4@wq" $VI -e 'ex.c'

# Patch: vi.c
EXINIT="rcm:|sc! @|vis 6@1816c 				fprintf(stderr, \"Nextvi-4.0 Usage: %s [-emRsv] [file ...]\\n\", argv[0]);
.
@vis 4@vis 6@1813a 			else if (argv[i][j] == 'R')
				readonly = 1;
.
@vis 4@wq" $VI -e 'vi.c'

# Patch: vi.h
EXINIT="rcm:|sc! @|vis 6@541a extern char readonly;
.
@vis 4@vis 6@408a 	char readonly;			/* read only */
.
@vis 4@wq" $VI -e 'vi.h'
