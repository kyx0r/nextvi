...
\\\\(\\\\[0-9\\\\]\\\\*\\\\)\\\\?\\\\)\\\\(\\\\?:\\\\(\\\\[-\\\\*-\\\\+/%\\\\]\\\\)\\\\(\\\\[0-9\\\\]\\\\+\\\\)\\\\)\\\\*\\\\(\\\\?:\\\\\\\\\\\\\\\\\\\\|\\\\.\\\\*\\\\?\\\\(\\\\?:\\\\(\\\\?<\\\\^\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\)\\\\\\\\\\\\\\\\\\\\|\\\\|\\\\\$\\\\)\\\\)\\\\*\\\\[ \\\\\\\\t\\\\]\\\\*\\\\)\\\\*\\\\)\\\\\\\\
\\\\(\\\\(pac\\\\|pr\\\\|ai\\\\|ish\\\\|err\\\\|ic\\\\|grp\\\\|mpt\\\\|rcm\\\\|shape\\\\|seq\\\\|ts\\\\|td\\\\|order\\\\|hl\\\\[lwpr\\\\]\\\\?\\\\|left\\\\|lim\\\\|led\\\\|vis\\\\)\\\\\\\\${SEP}??!${DBG:-.-5,.+5p\\${SEP}p FAIL line 261\\${SEP}vis 2\\${SEP}q! 1}${SEP};=
...
.
${SEP}.,\$f> \\\\{${SEP}??!${DBG:-.-5,.+5p\\${SEP}p FAIL line 364\\${SEP}vis 2\\${SEP}q! 1}${SEP};=
${SEP}.a 	/* window offset for vsplit (not for prompts) */
	int winx = (ai_max >= 0 && curwin) ? curwin->x : 0;
	int winw = (ai_max >= 0 && curwin) ? curwin->w : xcols;
...
${SEP}.+1;22;27c winh
.
${SEP}.,\$f> 		if \\\\(vi_lncol\\\\) \\\\{${SEP}??!${DBG:-.-5,.+5p\\${SEP}p FAIL line 368\\${SEP}vis 2\\${SEP}q! 1}${SEP};=
${SEP}.+1,#+1c 			term_pos(r, winx);
...
${SEP}.,\$;f> 		\\\\}
		...
		led_crender\\\\(lbuf_get\\\\(xb, nl\\\\) \\\\? lbuf_get\\\\(xb, nl\\\\) : \"~\", r,${SEP}??!${DBG:-.-5,.+5p\\${SEP}p FAIL line 384\\${SEP}vis 2\\${SEP}q! 1}${SEP};=
...
${SEP}.,\$;f> 					term_done\\\\(\\\\);
					term_init\\\\(\\\\);
				\\\\}${SEP}??!${DBG:-.-5,.+5p\\${SEP}p FAIL line 1542\\${SEP}vis 2\\${SEP}q! 1}${SEP};=
${SEP}.+2a 				if (xrows != orows || xcols != ocols)
					win_size();
.
${SEP}.,\$;f> 				xrows -= vi_status;
				vi_mod \\\\|= 1;${SEP}??!${DBG:-.-5,.+5p\\${SEP}p FAIL line 1545\\${SEP}vis 2\\${SEP}q! 1}${SEP};=
${SEP}.+2;10c  }
.
${SEP}.,\$;f> 		\\\\}
		if \\\\(vi_mod\\\\)
...
index a3d3876c..dcc69d66 100644
--- a/vi.c
+++ b/vi.c
...
+					xtop = MIN(lbuf_len(xb) - win_height(), xtop + n);
 				...
 				vi_mod |= 4;
 				break;
@@ -1401,6 +1514,24 @@ void vi(int init)
 				case 'v':
 					term_push(k == 'v' ? ":\x01" : ":\x02", 2); /* ^a : ^b */
 					break;
+				case 'c':
...
+					break;
 				case ';':
 					ln = vi_enprompt(":", "!", &k, &n);
 					goto do_excmd;
@@ -1532,7 +1663,8 @@ void vi(int init)
 				vi_mod |= 1;
 				break; }
 			case TK_CTL('z'):
-			case TK_CTL('l'):
...
+				int orows = xrows, ocols = xcols;
 				if (c == TK_CTL('z')) {
 					term_pos(xrows, 0);
 					term_suspend();
@@ -1540,9 +1672,11 @@ void vi(int init)
 					term_done();
 					term_init();
 				}
...
+				break; }
 			case 'm':
 				...
 				break;
@@ -1715,8 +1849,9 @@ void vi(int init)
 		}
 		if (vi_mod)
 			vi_col = vi_off2col(xb, xrow, xoff);
...
+			xleft = vi_col < _ww ? 0 : vi_col - _ww / 2; }
 		...
 		if (xmpt > 1) {
 			if (!xpln)
@@ -1758,19 +1893,35 @@ void vi(int init)
 			}
 		}
 		term_record = 1;
