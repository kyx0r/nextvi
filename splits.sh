...
# Patch: ex.c
EXINIT="rcm:|sc! #|vis 6#1570a /* window management functions */
...
.
#vis 4#1413d#vis 6#1391a 	{\"sp\", ec_split},
.
#vis 4#vis 6#1387a 	EO(vis),
	{\"vs\", ec_vsplit},
.
#vis 4#vis 6#1353a 	{\"eq\", ec_equalize},
.
#vis 4#vis 6#1326a static void *ec_split(char *loc, char *cmd, char *arg)
{
...
.
#vis 4#vis 6#159a 	/* update current window's buffer reference */
	if (curwin)
		curwin->buf = ex_buf;
.
#vis 4#vis 6#98a 	/* update current window's buffer reference */
	if (curwin)
		curwin->buf = ex_buf;
.
#vis 4#vis 6#43a struct win *wins;		/* head of window list */
...
.
#vis 4#vis 6#308,309c 	if (pos >= xleft + winw || pos < xleft)
		xleft = pos < winw ? 0 : pos - winw / 2;
.
#vis 4#vis 6#289a 	/* window offset for vsplit (not for prompts) */
	int winx = (ai_max >= 0 && curwin) ? curwin->x : 0;
	int winw = (ai_max >= 0 && curwin) ? curwin->w : xcols;
.
...
.
#vis 4#vis 6#1700,1701c 		{ int _ww = win_width();
...
#vis 4#vis 6#1535a 				if (xrows != orows || xcols != ocols)
					win_size();
.
#vis 4#vis 6#1528c 			case TK_CTL('l'): {
				int orows = xrows, ocols = xcols;
.
#vis 4#vis 6#1399a 				case 'c':
...
.
#vis 4#vis 6#1192a 		win_init();
.
#vis 4#vis 6#1181,1185c 	xtop = xtop - _wh / 2 > xrow ? \\
...
.
#vis 4#vis 6#225a /* draw all windows and separators */
...
.
#vis 4#vis 6#127a /* calculate screen row for buffer row in current window */
