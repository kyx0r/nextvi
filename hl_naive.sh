...
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}%;f> 	o = off\\\\[i\\\\]; \\\\\\\\
	if \\\\(o >= 0\\\\) \\\\{ \\\\\\\\
		for \\\\(l = i; off\\\\[i\\\\] == o; i\\\\+\\\\+\\\\); \\\\\\\\${SEP}??!${DBG:-re p FAIL line 117\\${SEP}p FAIL line 117${INTR}${QF}}${SEP};=
${SEP}+3${SEP}s/bound \\\\? ctt\\\\[atti\\\\+\\\\+\\\\] : //${SEP}??!${DBG:-re p FAIL line 117\\${SEP}p FAIL line 117${INTR}${QF}}${SEP}.,\$;f> 		return;
	...
	int j, c, l, i, o, n = r->n;${SEP}??!${DBG:-re p FAIL line 148\\${SEP}p FAIL line 148${INTR}${QF}}${SEP};=
...
${SEP}.,\$;f> 		for \\\\(c = cbeg; c < cend; c\\\\+\\\\+\\\\)
	...
	\\\\}${SEP}??!${DBG:-re p FAIL line 165\\${SEP}p FAIL line 165${INTR}${QF}}${SEP};=
${SEP}+3,#+38d${SEP}.,\$f> 	if \\\\(xhl\\\\)${SEP}??!${DBG:-re p FAIL line 205\\${SEP}p FAIL line 205${INTR}${QF}}${SEP};=
${SEP}+1,#+1c 		syn_highlight(att, s0, n);
.
${SEP}.,\$;f> 		for \\\\(; \\\\(char\\\\*\\\\)p < &led_attsb->s\\\\[led_attsb->s_n\\\\]; p\\\\+\\\\+\\\\) \\\\{
			if \\\\(p->s != s0 && p->s\\\\)
				continue;${SEP}??!${DBG:-re p FAIL line 212\\${SEP}p FAIL line 212${INTR}${QF}}${SEP};=
${SEP}+3,#+18c 			att[p->off] = syn_merge(att[p->off], p->att);
.
${SEP}.,\$;f> 				continue;
			if \\\\(r->pos\\\\[o \\\\+ 1\\\\] \\\\+ r->wid\\\\[o \\\\+ 1\\\\] != r->pos\\\\[o\\\\]\\\\)
				continue;${SEP}??!${DBG:-re p FAIL line 243\\${SEP}p FAIL line 243${INTR}${QF}}${SEP};=
