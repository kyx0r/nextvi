#!/bin/sh -e
# Generated by patch2vi from unified diff
# Available separators: 0001-0010 0013-0014 0016-0037 0102 0104 0112-0113 0117-0121 0126 0131-0132 0136 0176-0377

# Pass any argument to use patch(1) instead of nextvi ex commands
if [ -n "$1" ]; then
    sed '1,/^=== PATCH2VI PATCH ===$/d' "$0" | patch -p1 --merge=diff3
    exit $?
fi

VI=${VI:-vi}
if ! $VI -? 2>&1 | grep -q 'Nextvi'; then
    echo "Error: $VI is not nextvi" >&2
    echo "Set VI environment variable to point to nextvi binary" >&2
    exit 1
fi

SEP="$(printf '\001')"
# Command that handles readability line breaks
LB="0?"
# Phase 1 (search/mark): errors disabled by default,
# DBG1=1 enables error reporting, QF1=1 quits on failure
[ "$DBG1" = "1" ] && DBG1= || DBG1="0\?"
[ "$QF1" = "1" ] && QF1="\\${SEP}vis 2\\${SEP}q!1" || QF1=
# Phase 2 (edits): DBG2=1 disables errors, QF2=1 ignores them
[ "$DBG2" = "1" ] && DBG2="0\?" || DBG2=
[ "$QF2" = "1" ] && QF2= || QF2="\\${SEP}vis 2\\${SEP}q!1"
# Enters vi at failing code line in this script
# Designed for state inspection mid execution
[ "$INTR" = "1" ] && INTR="\\${SEP}|sc|\\${SEP}vis 2:0reg:e $0:83reg %@/:%f> %@p:&Q:b0:|sc! \\\\\\${SEP}|:vis 3\\${SEP}q1" || INTR=

# Patch: ex.c regex.c ren.c vi.c vi.h
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}98reg${SEP}b0${SEP}%ya b${SEP}%;f> 	xgrec--;
}

${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL ex.c:1762\\${SEP}pr${INTR}${QF1}}${SEP}${LB}
${SEP}+2m 0${SEP}${LB}
${SEP}'0a void ex_done(void)
{
	for (int i = 0; i < LEN(tempbufs); i++)
		if (tempbufs[i].lb) {
			free(tempbufs[i].path);
			lbuf_free(tempbufs[i].lb);
		}
	for (int i = 0; i < xbufcur; i++)
		bufs_free(i);
	for (int i = 0; i < LEN(xregs); i++)
		if (xregs[i])
			sbuf_free(xregs[i])
	rset_free(xkwdrs);
	free(bufs);
}

${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL ex.c:1762:m0\\${SEP}pr${INTR}${QF2}}${SEP}b1${SEP}%ya b${SEP};0${SEP}0reg${SEP}.,\$f> 	unsigned int sdense\\\\[prog->sparsesz\\\\], sparsesz = 0;${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL regex.c:638\\${SEP}pr${INTR}${QF1}}${SEP}98reg${SEP}${LB}
${SEP}m 0${SEP}${LB}
${SEP}'0a 	memset(sdense, 0, sizeof(int) * prog->sparsesz);
${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL regex.c:638:m0\\${SEP}pr${INTR}${QF2}}${SEP}b2${SEP}%ya b${SEP};0${SEP}0reg${SEP}.,\$f> ren_state rstates${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL ren.c:88\\${SEP}pr${INTR}${QF1}}${SEP}98reg${SEP}${LB}
${SEP}m 0${SEP}%;f+ 		pats\\\\[i\\\\] = fts\\\\[i\\\\]\\\\.pat;
	syn_ftrs = rset_make\\\\(i, pats, 0\\\\);
}${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL ren.c:444\\${SEP}pr${INTR}${QF1}}${SEP}${LB}
${SEP}+2m 1${SEP}${LB}
${SEP}'0a void ren_done(void)
{
	rset_free(dir_rslr);
	rset_free(dir_rsrl);
	rset_free(dir_rsctx);
	for (int i = 0; i < LEN(rstates); i++) {
		if (rstate[i].col) {
			free(rstate[i].col - 2);
			free(rstate[i].pos);
		}
	}
}

${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL ren.c:88:m0\\${SEP}pr${INTR}${QF2}}${SEP}${LB}
${SEP}'1a 
void syn_done(void)
{
	for (ftmidx--; ftmidx >= 0; ftmidx--)
		rset_free(ftmap[ftmidx].rs);
	free(ftmap);
	rset_free(syn_ftrs);
}
${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL ren.c:444:m1\\${SEP}pr${INTR}${QF2}}${SEP}b3${SEP}%ya b${SEP}%;f> 	else
		vi\\\\(1\\\\);
	term_done\\\\(\\\\);${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.c:1875\\${SEP}pr${INTR}${QF1}}${SEP}${LB}
${SEP}+2m 0${SEP}${LB}
${SEP}'0a 	ex_done();
	syn_done();
	ren_done();
	if (led_attsb)
		sbuf_free(led_attsb)
	free(ibuf);
${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.c:1875:m0\\${SEP}pr${INTR}${QF2}}${SEP}b4${SEP}%ya b${SEP};0${SEP}0reg${SEP}.,\$f> /\\\\* text direction \\\\*/${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.h:222\\${SEP}pr${INTR}${QF1}}${SEP}98reg${SEP}${LB}
${SEP}m 0${SEP};0${SEP}0reg${SEP}.,\$f+ syn_init${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.h:264\\${SEP}pr${INTR}${QF1}}${SEP}98reg${SEP}${LB}
${SEP}m 1${SEP};0${SEP}0reg${SEP}.,\$f+ ex_init${SEP}??!${DBG1:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.h:482\\${SEP}pr${INTR}${QF1}}${SEP}98reg${SEP}${LB}
${SEP}m 2${SEP}${LB}
${SEP}'0a void dir_done(void);
${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.h:222:m0\\${SEP}pr${INTR}${QF2}}${SEP}${LB}
${SEP}'1a void syn_done(void);
${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.h:264:m1\\${SEP}pr${INTR}${QF2}}${SEP}${LB}
${SEP}'2a void ex_done(void);
${SEP}??!${DBG2:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.h:482:m2\\${SEP}pr${INTR}${QF2}}${SEP}vis 2${SEP}b0${SEP}w${SEP}b1${SEP}w${SEP}b2${SEP}w${SEP}b3${SEP}w${SEP}b4${SEP}w${SEP}q" $VI -e 'ex.c' 'regex.c' 'ren.c' 'vi.c' 'vi.h'

exit 0
=== PATCH2VI DELTA ===
=== DELTA regex.c ===
=== GROUP 1 ===
+	memset(sdense, 0, sizeof(int) * prog->sparsesz);
=== END ===
=== LEVEL 2 ===
=== pre_ctx ===
	int si = 0, clistidx = 0, nlistidx, mcont = MATCH;
	int eol_ch = flg & REG_NEWLINE ? '\n' : 0;
	unsigned int sdense[prog->sparsesz], sparsesz = 0;
=== END ===
=== post_ctx ===
	char nsubs[prog->sub];
	for (i = 0; i < prog->laidx; i++)
		lb[i] = NULL;
=== END ===
=== pattern ===
	unsigned int sdense\[prog->sparsesz\], sparsesz = 0;
=== END ===
=== edit_cmd_rel ===
a 	memset(sdense, 0, sizeof(int) * prog->sparsesz);
=== END ===
=== END ===
=== DELTA ren.c ===
=== GROUP 1 ===
+void ren_done(void)
+{
+	rset_free(dir_rslr);
+	rset_free(dir_rsrl);
+	rset_free(dir_rsctx);
+	for (int i = 0; i < LEN(rstates); i++) {
+		if (rstate[i].col) {
+			free(rstate[i].col - 2);
+			free(rstate[i].pos);
+		}
+	}
+}
+
=== END ===
=== LEVEL 2 ===
=== pre_ctx ===
}

ren_state rstates[3]; /* 0 = current line, 1 = all other lines, 2 = aux rendering */
=== END ===
=== post_ctx ===
ren_state *rstate = rstates;

/* specify the screen position of the characters in s */
=== END ===
=== pattern ===
ren_state rstates
=== END ===
=== edit_cmd_rel ===
a void ren_done(void)
{
	rset_free(dir_rslr);
	rset_free(dir_rsrl);
	rset_free(dir_rsctx);
	for (int i = 0; i < LEN(rstates); i++) {
		if (rstate[i].col) {
			free(rstate[i].col - 2);
			free(rstate[i].pos);
		}
	}
}

=== END ===
=== END ===
=== DELTA vi.h ===
=== GROUP 1 ===
+void dir_done(void);
=== END ===
=== LEVEL 2 ===
=== pre_ctx ===
int ren_off(char *s, int p);
char *ren_translate(char *s, char *ln);
/* text direction */
=== END ===
=== post_ctx ===
int dir_context(char *s);
void dir_init(void);
/* syntax highlighting */
=== END ===
=== pattern ===
/\* text direction \*/
=== END ===
=== edit_cmd_rel ===
a void dir_done(void);
=== END ===
=== GROUP 2 ===
+void syn_done(void);
=== END ===
=== LEVEL 2 ===
=== pre_ctx ===
int syn_findhl(int id);
int syn_addhl(char *reg, int id);
void syn_init(void);
=== END ===
=== post_ctx ===

/* uc.c: utf-8 helper functions */
extern unsigned char utf8_length[256];
=== END ===
=== pattern ===
syn_init
=== END ===
=== edit_cmd_rel ===
a void syn_done(void);
=== END ===
=== GROUP 3 ===
+void ex_done(void);
=== END ===
=== LEVEL 2 ===
=== pre_ctx ===
#define ex_cprint2(line, ft, r, c, left, flg) { RS(2, ex_cprint(line, ft, r, c, left, flg)); }
#define ex_print(line, ft) { RS(2, ex_cprint(line, ft, -1, 0, 0, 1)); }
void ex_init(char **files, int n);
=== END ===
=== post_ctx ===
void ex_bufpostfix(struct buf *p, int clear);
int ex_krs(rset **krs, int *dir);
void ex_krsset(char *kwd, int dir);
=== END ===
=== pattern ===
ex_init
=== END ===
=== edit_cmd_rel ===
a void ex_done(void);
=== END ===
=== END ===
=== PATCH2VI PATCH ===
diff --git a/ex.c b/ex.c
index bc6a6269..c436026c 100644
--- a/ex.c
+++ b/ex.c
@@ -1760,6 +1760,22 @@ void ex(void)
 	xgrec--;
 }
 
+void ex_done(void)
+{
+	for (int i = 0; i < LEN(tempbufs); i++)
+		if (tempbufs[i].lb) {
+			free(tempbufs[i].path);
+			lbuf_free(tempbufs[i].lb);
+		}
+	for (int i = 0; i < xbufcur; i++)
+		bufs_free(i);
+	for (int i = 0; i < LEN(xregs); i++)
+		if (xregs[i])
+			sbuf_free(xregs[i])
+	rset_free(xkwdrs);
+	free(bufs);
+}
+
 void ex_init(char **files, int n)
 {
 	xbufsalloc = MAX(n, xbufsalloc);
diff --git a/regex.c b/regex.c
index ff88bb41..42399e6a 100644
--- a/regex.c
+++ b/regex.c
@@ -636,6 +636,7 @@ static int re_pikevm(rcode *prog, const char *s, const char **subp, int nsubc, i
 	int si = 0, clistidx = 0, nlistidx, mcont = MATCH;
 	int eol_ch = flg & REG_NEWLINE ? '\n' : 0;
 	unsigned int sdense[prog->sparsesz], sparsesz = 0;
+	memset(sdense, 0, sizeof(int) * prog->sparsesz);
 	char nsubs[prog->sub];
 	for (i = 0; i < prog->laidx; i++)
 		lb[i] = NULL;
diff --git a/ren.c b/ren.c
index 9b4776c8..3eb1a39b 100644
--- a/ren.c
+++ b/ren.c
@@ -86,6 +86,19 @@ static int ren_cwid(char *s, int pos)
 }
 
 ren_state rstates[3]; /* 0 = current line, 1 = all other lines, 2 = aux rendering */
+void ren_done(void)
+{
+	rset_free(dir_rslr);
+	rset_free(dir_rsrl);
+	rset_free(dir_rsctx);
+	for (int i = 0; i < LEN(rstates); i++) {
+		if (rstate[i].col) {
+			free(rstate[i].col - 2);
+			free(rstate[i].pos);
+		}
+	}
+}
+
 ren_state *rstate = rstates;
 
 /* specify the screen position of the characters in s */
@@ -442,3 +455,11 @@ void syn_init(void)
 		pats[i] = fts[i].pat;
 	syn_ftrs = rset_make(i, pats, 0);
 }
+
+void syn_done(void)
+{
+	for (ftmidx--; ftmidx >= 0; ftmidx--)
+		rset_free(ftmap[ftmidx].rs);
+	free(ftmap);
+	rset_free(syn_ftrs);
+}
diff --git a/vi.c b/vi.c
index d133d031..139da88d 100644
--- a/vi.c
+++ b/vi.c
@@ -1873,6 +1873,12 @@ int main(int argc, char *argv[])
 	else
 		vi(1);
 	term_done();
+	ex_done();
+	syn_done();
+	ren_done();
+	if (led_attsb)
+		sbuf_free(led_attsb)
+	free(ibuf);
 	if (xvis & 8)
 		term_scrl;
 	return abs(xquit) - 1;
diff --git a/vi.h b/vi.h
index 98f80e03..15cf0517 100644
--- a/vi.h
+++ b/vi.h
@@ -220,6 +220,7 @@ int ren_noeol(char *s, int p);
 int ren_off(char *s, int p);
 char *ren_translate(char *s, char *ln);
 /* text direction */
+void dir_done(void);
 int dir_context(char *s);
 void dir_init(void);
 /* syntax highlighting */
@@ -262,6 +263,7 @@ void syn_reloadft(int hl, int flg);
 int syn_findhl(int id);
 int syn_addhl(char *reg, int id);
 void syn_init(void);
+void syn_done(void);
 
 /* uc.c: utf-8 helper functions */
 extern unsigned char utf8_length[256];
@@ -480,6 +482,7 @@ void ex_cprint(char *line, char *ft, int r, int c, int left, int flg);
 #define ex_cprint2(line, ft, r, c, left, flg) { RS(2, ex_cprint(line, ft, r, c, left, flg)); }
 #define ex_print(line, ft) { RS(2, ex_cprint(line, ft, -1, 0, 0, 1)); }
 void ex_init(char **files, int n);
+void ex_done(void);
 void ex_bufpostfix(struct buf *p, int clear);
 int ex_krs(rset **krs, int *dir);
 void ex_krsset(char *kwd, int dir);
