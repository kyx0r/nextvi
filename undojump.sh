...
# Available separators: 0003-0010 0013-0014 0016-0037 0102 0104-0105 0107-0110 0112 0120-0123 0126-0127 0131-0132 0136 0176-0377
	...
	if (hist_u) {
		do {
			...
			while (hist_u && lb->hist[hist_u - 1].seq == useq)
				lo = &lb->hist[--hist_u];
		} while (hist_u && hist_u >= lb->hist_j);
		lb->hist_j = hist_u;
		hist_u = lb->hist_u;
		while (hist_u != lb->hist_j) {
			lo = &lb->hist[--hist_u];
			if (lb->hist[lb->hist_j].pos > lo->pos) {
				rpos += lo->n_ins;
				rpos -= lo->n_del;
			}
		}
		...
		return ret ? ret : 1;
	*pos = rpos + lb->hist[lb->hist_j].pos;
	*off = lo->pos_off;
	return ret;
}
...
index 03ed7b03..412559a9 100644
--- a/vi.c
+++ b/vi.c
...
index 7c7d9e2c..85734d58 100644
--- a/vi.h
+++ b/vi.h
@@ -150,6 +150,7 @@ struct lbuf {
 	...
 	int hist_n;			/* current history head in hist[] */
 	int hist_u;			/* current undo head in hist[] */
+	int hist_j;			/* undo jump head in hist[] */
 };
 #define lbuf_len(lb) lb->ln_n
