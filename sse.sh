...
\\\\{${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 6\\${SEP}${QF}}${SEP};=
...
${SEP}.,\$;f> 	register const char \\\\*i;
	for \\\\(i=s; \\\\*i && \\\\*i != delim; \\\\+\\\\+i\\\\);
	return i-s;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 9\\${SEP}${QF}}${SEP};=
${SEP}+2a #endif
.
...
EXINIT="rcm:|sc! \\\\${SEP}|vis 3${SEP}%;f> 		rstate->holelen = uc_len\\\\(ss\\\\);
		...
		memset\\\\(ss, 0, rstate->holelen\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 111\\${SEP}${QF}}${SEP};=
${SEP}+3,#+1c 	} else {
		n = 0;
#ifdef __SSE2__
		...
		for (; (l = uc_len(ss)); n++)
.
${SEP}.,\$f> 			ss \\\\+= l;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 113\\${SEP}${QF}}${SEP};=
${SEP}.a count_done:;
	}
.
