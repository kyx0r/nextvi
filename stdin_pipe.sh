...
# Patch: ex.c
EXINIT="rcm:|sc! @|vis 6@1581a 	signal(SIGINT, SIG_DFL); /* got past init? ok remove ^c */
.
@vis 4@vis 6@1580a 	if (stdin_fd) {
		if (i)
			ec_edit(NULL, \"\", \"\");
...
.
@vis 4@vis 6@1578c 		ec_edit(!n && stdin_fd ? NULL : \"\", \"e\", s);
.
@vis 4@vis 6@1575a 	int i = n;
.
@vis 4@vis 6@1573c 	xbufsalloc = MAX(n + !!stdin_fd, xbufsalloc);
.
