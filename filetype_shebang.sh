...
# Patch: ex.c
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}b0${SEP}%;f> 			fd < 0 \\\\|\\\\| rd \\\\? 'f' : 'r'\\\\);
	if \\\\(!\\\\(xvis & 4\\\\)\\\\)
		ex_print\\\\(msg, bar_ft\\\\)${SEP}??!${DBG:-ya!p\\${SEP}prp\\${SEP}p FAIL ex.c:380\\${SEP}pr${INTR}${QF}}${SEP}${LB}
...
index 0ec68c95..14233e5e 100644
--- a/ex.c
+++ b/ex.c
