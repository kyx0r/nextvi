#!/bin/sh -e
# Generated by patch2vi from unified diff
# Available separators: 0001-0010 0013-0014 0016-0037 0104 0112-0113 0121 0126 0131-0132 0136 0177-0377

# Pass any argument to use patch(1) instead of nextvi ex commands
if [ -n "$1" ]; then
    sed '1,/^=== PATCH2VI PATCH ===$/d' "$0" | patch -p1 --merge=diff3
    exit $?
fi

VI=${VI:-vi}
if ! $VI -? 2>&1 | grep -q 'Nextvi'; then
    echo "Error: $VI is not nextvi" >&2
    echo "Set VI environment variable to point to nextvi binary" >&2
    exit 1
fi

SEP="$(printf '\001')"
# Comment to continue despite errors (errors are still printed)
QF="\\${SEP}vis 2\\${SEP}q!1"
# Uncomment to enter interactive vi on patch failure
#INTR="\\${SEP}|sc|\\${SEP}vis 2:e $0:%f>:@Q:q!1"
# Uncomment to skip errors (0? = silent nop)
#DBG="0\?"

# Patch: ex.c
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}%;f> sbuf \\\\*xacreg;			/\\\\* autocomplete db filter regex \\\\*/
rset \\\\*xkwdrs;			/\\\\* the last searched keyword rset \\\\*/
sbuf \\\\*xregs\\\\[256\\\\];		/\\\\* string registers \\\\*/${SEP}??!${DBG:-re p FAIL line 41\\${SEP}p FAIL line 41${INTR}${QF}}${SEP};=
${SEP}+2a int xexrc = 0;			/* read .exrc from the current directory */
.
${SEP}.,\$f> EO\\\\(pac\\\\)${SEP}??!${DBG:-re p FAIL line 1438\\${SEP}p FAIL line 1438${INTR}${QF}}${SEP};=
${SEP}+2a EO(exrc)
.
${SEP}.,\$;f> 	EO\\\\(ai\\\\),
	\\\\{\"ac\", ec_setacreg\\\\},
	\\\\{\"a\", ec_insert\\\\},${SEP}??!${DBG:-re p FAIL line 1478\\${SEP}p FAIL line 1478${INTR}${QF}}${SEP};=
${SEP}+2a 	EO(exrc),
.
${SEP}.,\$;f> 	xgrec--;
\\\\}

${SEP}??!${DBG:-re p FAIL line 1712\\${SEP}p FAIL line 1712${INTR}${QF}}${SEP};=
${SEP}+2a void ex_script(FILE *fp)
{
	char done = 0;
	do {
		size_t n = 128, i = 0;
		int c;
		char *ln = malloc(128);
		while ((c = fgetc(fp)) != EOF && c != '\\\\n') {
			if (i >= n - 2) {
				n += 128;
				ln = erealloc(ln, n);
			}
			ln[i++] = c;
		}
		if (!i) {
			free(ln);
			done = 1;
			break;
		}
		ln[i] = '\\\\0';
		ex_command(ln);
		free(ln);
	} while(!done);
}

void load_exrc(char *exrc)
{
	struct stat st;
	if (stat(exrc, &st) == 0) {
		if (st.st_uid == getuid() && !(st.st_mode & S_IWGRP) && !(st.st_mode & S_IWOTH)) {
			FILE *fp = fopen(exrc, \"r\");
			if (fp) {
				ex_script(fp);
				fclose(fp);
			} else {
				fprintf(stderr, \"Cannot open ~/.exrc\\\\n\");
				exit(EXIT_FAILURE);
			}
		} else {
			fprintf(stderr, \"Bad permissions on ~/.exrc\\\\n\");
			exit(EXIT_FAILURE);
		}
	}
}

.
${SEP}.,\$;f> 		s = \\\\*\\\\(\\\\+\\\\+files\\\\);
	\\\\} while \\\\(--n > 0\\\\);
	xvis &= ~4;${SEP}??!${DBG:-re p FAIL line 1724\\${SEP}p FAIL line 1724${INTR}${QF}}${SEP};=
${SEP}+3${SEP}s/\\\\)\\\\)\\\\)/))) {/${SEP}??!${DBG:-re p FAIL line 1724\\${SEP}p FAIL line 1724${INTR}${QF}}${SEP}.,\$f> 		ex_command\\\\(s\\\\)${SEP}??!${DBG:-re p FAIL line 1725\\${SEP}p FAIL line 1725${INTR}${QF}}${SEP};=
${SEP}.a 	} else {
		char *homeenv = getenv(\"HOME\");
		if (homeenv) {
			char exrc[PATH_MAX];
			snprintf(exrc, sizeof(exrc), \"%s/.exrc\", homeenv);
			load_exrc(exrc);
		}
	}
	if (xexrc) {
		char buf[PATH_MAX];
		getcwd(buf, PATH_MAX);
		if (strcmp(buf, getenv(\"HOME\")) != 0)
			load_exrc(\".exrc\");
	}
.
${SEP}vis 2${SEP}wq" $VI -e 'ex.c'

exit 0
=== PATCH2VI DELTA ===
=== DELTA ex.c ===
--- patch2vi_5nVqGg_ex.c.diff.orig	2026-04-23 11:50:18.103815999 -0100
+++ patch2vi_5nVqGg_ex.c.diff	2026-04-23 11:50:29.852694175 -0100
@@ -25,9 +25,7 @@
 #rel
 .,\$;f>
 === SEARCH PATTERN ===
-EO\(pac\) EO\(pr\) EO\(ai\) EO\(err\) EO\(ish\) EO\(ic\) EO\(mpt\) EO\(rcm\)
-EO\(shape\) EO\(seq\) EO\(ts\) EO\(td\) EO\(order\) EO\(hll\) EO\(hlw\)
-EO\(hlp\) EO\(hlr\) EO\(hl\) EO\(lim\) EO\(led\) EO\(vis\)
+EO\(pac\)
 --- extra (delete to include) ---
 
 _EO\(grp, xgrp = \(!\*arg \? !xgrp : eo_val\(arg\)\) \* 2; return NULL;\)
=== PATCH2VI PATCH ===
diff --git a/ex.c b/ex.c
index 36b8a6d6..1b055122 100644
--- a/ex.c
+++ b/ex.c
@@ -39,6 +39,7 @@ int xexec_dep;			/* ex_exec recursion depth */
 sbuf *xacreg;			/* autocomplete db filter regex */
 rset *xkwdrs;			/* the last searched keyword rset */
 sbuf *xregs[256];		/* string registers */
+int xexrc = 0;			/* read .exrc from the current directory */
 struct buf *bufs;		/* main buffers */
 struct buf tempbufs[3];		/* temporary buffers, for internal use */
 struct buf *ex_buf;		/* current buffer */
@@ -1436,6 +1437,7 @@ static void *eo_##opt(char *loc, char *cmd, char *arg) { inner }
 EO(pac) EO(pr) EO(ai) EO(err) EO(ish) EO(ic) EO(mpt) EO(rcm)
 EO(shape) EO(seq) EO(ts) EO(td) EO(order) EO(hll) EO(hlw)
 EO(hlp) EO(hlr) EO(hl) EO(lim) EO(led) EO(vis)
+EO(exrc)
 
 _EO(grp, xgrp = (!*arg ? !xgrp : eo_val(arg)) * 2; return NULL;)
 
@@ -1476,6 +1478,7 @@ static struct excmd {
 	EO(ai),
 	{"ac", ec_setacreg},
 	{"a", ec_insert},
+	EO(exrc),
 	EO(err),
 	{"ef!", ec_fuzz},
 	{"ef", ec_fuzz},
@@ -1710,6 +1713,51 @@ void ex(void)
 	xgrec--;
 }
 
+void ex_script(FILE *fp)
+{
+	char done = 0;
+	do {
+		size_t n = 128, i = 0;
+		int c;
+		char *ln = malloc(128);
+		while ((c = fgetc(fp)) != EOF && c != '\n') {
+			if (i >= n - 2) {
+				n += 128;
+				ln = erealloc(ln, n);
+			}
+			ln[i++] = c;
+		}
+		if (!i) {
+			free(ln);
+			done = 1;
+			break;
+		}
+		ln[i] = '\0';
+		ex_command(ln);
+		free(ln);
+	} while(!done);
+}
+
+void load_exrc(char *exrc)
+{
+	struct stat st;
+	if (stat(exrc, &st) == 0) {
+		if (st.st_uid == getuid() && !(st.st_mode & S_IWGRP) && !(st.st_mode & S_IWOTH)) {
+			FILE *fp = fopen(exrc, "r");
+			if (fp) {
+				ex_script(fp);
+				fclose(fp);
+			} else {
+				fprintf(stderr, "Cannot open ~/.exrc\n");
+				exit(EXIT_FAILURE);
+			}
+		} else {
+			fprintf(stderr, "Bad permissions on ~/.exrc\n");
+			exit(EXIT_FAILURE);
+		}
+	}
+}
+
 void ex_init(char **files, int n)
 {
 	xbufsalloc = MAX(n, xbufsalloc);
@@ -1721,6 +1769,20 @@ void ex_init(char **files, int n)
 		s = *(++files);
 	} while (--n > 0);
 	xvis &= ~4;
-	if ((s = getenv("EXINIT")))
+	if ((s = getenv("EXINIT"))) {
 		ex_command(s)
+	} else {
+		char *homeenv = getenv("HOME");
+		if (homeenv) {
+			char exrc[PATH_MAX];
+			snprintf(exrc, sizeof(exrc), "%s/.exrc", homeenv);
+			load_exrc(exrc);
+		}
+	}
+	if (xexrc) {
+		char buf[PATH_MAX];
+		getcwd(buf, PATH_MAX);
+		if (strcmp(buf, getenv("HOME")) != 0)
+			load_exrc(".exrc");
+	}
 }
