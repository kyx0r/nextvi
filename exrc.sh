#!/bin/sh
# Generated by patch2vi from unified diff
# Available separators: 
set -e

# Path to nextvi (adjust as needed)
VI=${VI:-vi}

# Verify that VI is nextvi
if ! $VI -? 2>&1 | grep -q 'Nextvi'; then
    echo "Error: $VI is not nextvi" >&2
    echo "Set VI environment variable to point to nextvi" >&2
    exit 1
fi

# Patch: ex.c
SEP="$(printf '\x01')"
EXINIT="rcm:|sc! \\\\${SEP}|vis 6${SEP}%;f> sbuf \\\\*xacreg;			/\\\\* autocomplete db filter regex \\\\*/
rset \\\\*xkwdrs;			/\\\\* the last searched keyword rset \\\\*/
sbuf \\\\*xregs\\\\[256\\\\];		/\\\\* string registers \\\\*/${SEP}??!.-5,.+5p\\${SEP}p FAIL line 40\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a int xexrc = 0;			/* read .exrc from the current directory */
.
${SEP}.,$;f+ EO\\\\(pac\\\\) EO\\\\(pr\\\\) EO\\\\(ai\\\\) EO\\\\(err\\\\) EO\\\\(ish\\\\) EO\\\\(ic\\\\) EO\\\\(grp\\\\) EO\\\\(mpt\\\\) EO\\\\(rcm\\\\)
EO\\\\(shape\\\\) EO\\\\(seq\\\\) EO\\\\(ts\\\\) EO\\\\(td\\\\) EO\\\\(order\\\\) EO\\\\(hll\\\\) EO\\\\(hlw\\\\)
EO\\\\(hlp\\\\) EO\\\\(hlr\\\\) EO\\\\(hl\\\\) EO\\\\(lim\\\\) EO\\\\(led\\\\) EO\\\\(vis\\\\)${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1343\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a EO(exrc)
.
${SEP}.,$;f+ 	EO\\\\(ai\\\\),
	\\\\{\"ac\", ec_setacreg\\\\},
	\\\\{\"a\", ec_insert\\\\},${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1381\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a 	EO(exrc),
.
${SEP}.,$;f+ 	xgrec--;
\\\\}
${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1599\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+2a void ex_script(FILE *fp)
{
	char done = 0;
	do {
		size_t n = 128, i = 0;
		int c;
		char *ln = malloc(128);
		while ((c = fgetc(fp)) != EOF && c != '\\\\n') {
			if (i >= n - 2) {
				n += 128;
				ln = erealloc(ln, n);
			}
			ln[i++] = c;
		}
		if (!i) {
			free(ln);
			done = 1;
			break;
		}
		ln[i] = '\\\\0';
		ex_command(ln);
		free(ln);
	} while(!done);
}

void load_exrc(char *exrc)
{
	struct stat st;
	if (stat(exrc, &st) == 0) {
		if (st.st_uid == getuid() && !(st.st_mode & S_IWGRP) && !(st.st_mode & S_IWOTH)) {
			FILE *fp = fopen(exrc, \"r\");
			if (fp) {
				ex_script(fp);
				fclose(fp);
			} else {
				fprintf(stderr, \"Cannot open ~/.exrc\\\\n\");
				exit(EXIT_FAILURE);
			}
		} else {
			fprintf(stderr, \"Bad permissions on ~/.exrc\\\\n\");
			exit(EXIT_FAILURE);
		}
	}
}

.
${SEP}.,$;f+ 		s = \\\\*\\\\(\\\\+\\\\+files\\\\);
	\\\\} while \\\\(--n > 0\\\\);
	xvis &= ~8;${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1611\\${SEP}vis 4\\${SEP}q!${SEP};=
${SEP}.+3;28c  {
.
${SEP}.-1${SEP}>		ex_command\\(s\\)>a 	} else {
		char *homeenv = getenv(\"HOME\");
		if (homeenv) {
			char exrc[PATH_MAX];
			snprintf(exrc, sizeof(exrc), \"%s/.exrc\", homeenv);
			load_exrc(exrc);
		}
	}
	if (xexrc) {
		char buf[PATH_MAX];
		getcwd(buf, PATH_MAX);
		if (strcmp(buf, getenv(\"HOME\")) != 0)
			load_exrc(\".exrc\");
	}
.
${SEP}??!.-5,.+5p\\${SEP}p FAIL line 1612\\${SEP}vis 4\\${SEP}q!${SEP}vis 4${SEP}wq" $VI -e 'ex.c'
