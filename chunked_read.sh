	...
	if (!init) {
		...
		while ((nr = read(fd, s + n, sz - n)) > 0) {
			...
			if (n >= sz + step) {
				...
				step = 0;
.
${SEP}.,\$f> 			\\\\}${SEP}??!${DBG:-re p FAIL line 218\\${SEP}p FAIL line 218${INTR}${QF}}${SEP};=
${SEP}+1,#+5d${SEP}.,\$f> 		\\\\}${SEP}??!${DBG:-re p FAIL line 224\\${SEP}p FAIL line 224${INTR}${QF}}${SEP};=
${SEP}.a 		s[n] = '\\\\0';
		lbuf_edit(lb, s, beg, end, 0, 0);
		free(s);
		return nr != 0;
.
${SEP}.,\$f> 	\\\\}${SEP}??!${DBG:-re p FAIL line 226\\${SEP}p FAIL line 226${INTR}${QF}}${SEP};=
...
void lbuf_free\\\\(struct lbuf \\\\*lb\\\\);${SEP}??!${DBG:-re p FAIL line 156\\${SEP}p FAIL line 156${INTR}${QF}}${SEP};=
