...
${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 64\\${SEP}${QF}}${SEP};=
${SEP}+3${SEP}s/\\\\(int\\\\[\\\\]\\\\)//${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 64\\${SEP}${QF}}${SEP}vis 2${SEP}wq" $VI -e 'conf.c'
...
${SEP}.,\$;f> 						soff \\\\+ offs\\\\[xgrp\\\\], &xrow, &xoff\\\\)\\\\)
			ret = xuerr;
		free\\\\(sb\\\\.s\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 528\\${SEP}${QF}}${SEP};=
${SEP}+2a 		free(offs);
.
...
${SEP}.,\$;f> 	if \\\\(rs != xkwdrs\\\\)
		rset_free\\\\(rs\\\\);
	free\\\\(rep\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1042\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(offs);
.
...
${SEP}.,\$;f> 				break;
			\\\\*o = _o;
			\\\\*r = i;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 508\\${SEP}${QF}}${SEP};=
${SEP}+3c 			if (dir > 0) {
				free(offs);
.
${SEP}.,\$f> 				return 0;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 509\\${SEP}${QF}}${SEP};=
${SEP}.a 			}
.
${SEP}.,\$;f> 		\\\\}
		off = 0;
	\\\\}${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 515\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(offs);
.
...
EXINIT="rcm:|sc! \\\\${SEP}|vis 3${SEP}%;f> 		xic \\\\? REG_ICASE \\\\| REG_NEWLINE : REG_NEWLINE\\\\);
	if \\\\(!rs\\\\)
		return;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 48\\${SEP}${QF}}${SEP};=
${SEP}+3c 	int *subs = emalloc(rs->nsubc * sizeof(int));
.
${SEP}.,\$;f> 	sbuf_null\\\\(acsb\\\\)
	free\\\\(ibuf->s\\\\);
	rset_free\\\\(rs\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 80\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(subs);
.
${SEP}.,\$;f> 	int att_old = 0, atti = 0, cterm = cend - cbeg;
	char \\\\*bound = NULL;
	char \\\\*\\\\*chrs = r->chrs;	/\\\\* chrs\\\\[i\\\\]: the i-th character in s0 \\\\*/${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 151\\${SEP}${QF}}${SEP};=
...
${SEP}.,\$;f> 		return 0;
	flg = prog->flg \\\\| flg;
	const char \\\\*sp = s, \\\\*_sp = s, \\\\*s0, \\\\*s1;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 628\\${SEP}${QF}}${SEP};=
...
.
${SEP}.,\$f> 	rsub \\\\*nsub, \\\\*sub, \\\\*matched = NULL, \\\\*freesub = NULL;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 631\\${SEP}${QF}}${SEP};=
${SEP}+1c 	rthread *_clist = emalloc(prog->len * sizeof(rthread)), *_nlist = emalloc(prog->len * sizeof(rthread));
.
${SEP}.,\$f> 	rthread \\\\*clist = _clist, \\\\*nlist = _nlist, \\\\*tmp;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 633\\${SEP}${QF}}${SEP};=
${SEP}+1${SEP}s/lb\\\\[prog->laidx\\\\+1\\\\]/*lb = emalloc(prog->laidx * sizeof(lb[0]))/${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 633\\${SEP}${QF}}${SEP}.,\$;f> 	int cnt, spc, i, c, j, osubp = nsubc \\\\* sizeof\\\\(char\\\\*\\\\);
	int si = 0, clistidx = 0, nlistidx, mcont = MATCH;
	int eol_ch = flg & REG_NEWLINE \\\\? '\\\\\\\\n' : 0;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 638\\${SEP}${QF}}${SEP};=
...
\\\\{${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 717\\${SEP}${QF}}${SEP};=
${SEP}+3c 	const char **subs = emalloc((rs->nsubc+2) * sizeof(subs[0]));
.
${SEP}.,\$;f> 						grps\\\\[gi \\\\+ 1\\\\] = -1;
					\\\\}
				\\\\}${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 733\\${SEP}${QF}}${SEP};=
${SEP}+2a 				free(subs);
.
${SEP}.,\$;f> 			\\\\}
		\\\\}
	\\\\}${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 737\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(subs);
.
...
\\\\{
	int i = n, set = hls\\\\[i\\\\]\\\\.set;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 254\\${SEP}${QF}}${SEP};=
${SEP}+3c 	char **pats = emalloc(hlslen * sizeof(char *));
.
${SEP}.,\$;f> 	ftmap\\\\[fti\\\\]\\\\.ft = name;
	ftmap\\\\[fti\\\\]\\\\.rs = rset_make\\\\(i - n, pats, flg\\\\);
	ftmap\\\\[fti\\\\]\\\\.seteidx = i;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 260\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(pats);
.
${SEP}.,\$;f> 	int fti = ftidx, blockhl = syn_blockhl, blockca = -1;
	re:;
	rset \\\\*rs = ftmap\\\\[fti\\\\]\\\\.rs;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 310\\${SEP}${QF}}${SEP};=
${SEP}+3${SEP}s/subs\\\\[rs->nsubc\\\\]/*subs = emalloc(rs->nsubc * sizeof(int))/${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 310\\${SEP}${QF}}${SEP}.,\$;f> 		flg = REG_NOTBOL;
	\\\\}
	fti\\\\+\\\\+;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 353\\${SEP}${QF}}${SEP};=
${SEP}+3c 	if (ftmidx > fti && ftmap[fti-1].ft == ftmap[fti].ft) {
		free(subs);
.
${SEP}.,\$f> 		goto re;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 354\\${SEP}${QF}}${SEP};=
${SEP}.a 	}
.
${SEP}.,\$;f> 	if \\\\(blockca >= 0 && SYN_BSSET\\\\(blockca\\\\) && !SYN_BESET\\\\(blockca\\\\) && last_scdir > 0\\\\)
		syn_blockhl = -1;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 357\\${SEP}${QF}}${SEP};=
${SEP}+2c 	if (syn_blockhl < 0 || blockhl < 0) {
		free(subs);
.
${SEP}.,\$f> 		return;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 358\\${SEP}${QF}}${SEP};=
${SEP}.a 	}
.
${SEP}.,\$;f> 	for \\\\(j = 0; j < n; j\\\\+\\\\+\\\\)
		if \\\\(!att\\\\[j\\\\] \\\\|\\\\| !SYN_BPSET\\\\(blockatt\\\\)\\\\)
			att\\\\[j\\\\] = blockatt;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 361\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(subs);
.
${SEP}.,\$;f> 
void syn_init\\\\(void\\\\)
\\\\{${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 405\\${SEP}${QF}}${SEP};=
${SEP}+3c 	char **pats = emalloc(ftslen * sizeof(char *));
.
${SEP}.,\$;f> 	for \\\\(; i < ftslen; i\\\\+\\\\+\\\\)
		pats\\\\[i\\\\] = fts\\\\[i\\\\]\\\\.pat;
	syn_ftrs = rset_make\\\\(i, pats, 0\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 409\\${SEP}${QF}}${SEP};=
${SEP}+2a 	free(pats);
.
...
SEP="$(printf '\001')"
QF=${QF-"$(printf 'vis 2\\\001q! 1')"}
EXINIT="rcm:|sc! \\\\${SEP}|vis 3${SEP}%;f> 			s = lbuf_get\\\\(xb, row - vi_rshift\\\\);
			goto skip;
...
${SEP}.,\$;f> 		restore\\\\(syn_blockhl\\\\)
		restore\\\\(xtd\\\\)
		restore\\\\(ftidx\\\\)${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 167\\${SEP}${QF}}${SEP};=
${SEP}+2a 		free(tmp);
		free(snum);
.
${SEP}.,\$;f> 				if \\\\(!\\\\(ln = lbuf_get\\\\(xb, xrow\\\\)\\\\)\\\\)
					break;
				ln \\\\+= xoff;${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1289\\${SEP}${QF}}${SEP};=
${SEP}+3c 				char *buf = emalloc(strlen(ln)+4);
.
${SEP}.,\$;f> 				strcpy\\\\(buf, \":e \"\\\\);
				strcpy\\\\(buf\\\\+3, ln\\\\);
				term_push\\\\(buf, strlen\\\\(ln\\\\)\\\\+3\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1292\\${SEP}${QF}}${SEP};=
${SEP}+2a 				free(buf);
.
${SEP}.,\$;f> 					goto do_excmd;
				case '/': \\\\{
					cs = vi_curword\\\\(xb, xrow, xoff, vi_arg, 1\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1404\\${SEP}${QF}}${SEP};=
${SEP}+3c 					char *buf = emalloc(cs ? strlen(cs)+30 : 30);
.
${SEP}.,\$;f> 						strcat\\\\(buf, cs\\\\);
					free\\\\(cs\\\\);
					ln = vi_enprompt\\\\(\":\", buf, &k, &n\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1409\\${SEP}${QF}}${SEP};=
${SEP}+2a 					free(buf);
.
${SEP}.,\$;f> 				case 't': \\\\{
					vi_drawmsg\\\\(\"arg2:\\\\(0\\\\|#\\\\)\"\\\\);
					cs = vi_curword\\\\(xb, xrow, xoff, vi_prefix\\\\(\\\\), 1\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1414\\${SEP}${QF}}${SEP};=
${SEP}+3c 					char *buf = emalloc(cs ? strlen(cs)+30 : 30);
.
${SEP}.,\$;f> 						free\\\\(cs\\\\);
					\\\\}
					ln = vi_enprompt\\\\(\":\", buf, &k, &n\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1423\\${SEP}${QF}}${SEP};=
${SEP}+2a 					free(buf);
.
${SEP}.,\$;f> 					goto do_excmd; \\\\}
				case 'r': \\\\{
					cs = vi_curword\\\\(xb, xrow, xoff, vi_arg, 1\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1427\\${SEP}${QF}}${SEP};=
${SEP}+3c 					char *buf = emalloc(cs ? strlen(cs)+30 : 30);
.
${SEP}.,\$;f> 						free\\\\(cs\\\\);
					\\\\}
					ln = vi_enprompt\\\\(\":\", buf, &k, &n\\\\);${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 1434\\${SEP}${QF}}${SEP};=
${SEP}+2a 					free(buf);
.
...
EXINIT="rcm:|sc! \\\\${SEP}|vis 3${SEP}%;f> struct highlight \\\\{
	...
	char \\\\*pat;		/\\\\* regular expression \\\\*/${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 498\\${SEP}${QF}}${SEP};=
${SEP}+3${SEP}s/\\\\*att/att[16]/${SEP}??!${DBG:--5,+5p\\${SEP}p FAIL line 498\\${SEP}${QF}}${SEP}vis 2${SEP}wq" $VI -e 'vi.h'

exit 0
=== PATCH2VI DELTA ===
=== PATCH2VI PATCH ===
diff --git a/conf.c b/conf.c
index 543211a1..f493e5e4 100644
--- a/conf.c
+++ b/conf.c
@@ -61,7 +61,7 @@ const int ftslen = LEN(fts);
...
index 95a85a2d..7ddf197c 100644
--- a/ex.c
+++ b/ex.c
...
index 0c984c04..f0f816b5 100644
--- a/vi.h
+++ b/vi.h
