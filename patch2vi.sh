...
# come back out of storage as they went in, gates and all, and those gates were
# derived against the base as it read then: once it moves under them they probe
# for text that is gone. So the blocks come off, the base is regenerated without
# them, and each block's stored === COMPAT PATCH === diff goes back to -co,
# which derives fresh gates against the base that now exists. The stored diff is
# verbatim, which is what lets it come back out as the patch its author handed
# in.
#
...
compat_order() (
	...
	/^My current EXINIT setup:$/	{
		...
		print "Compat order"
		print "--------------"
		print "A patch carrying a compat block for another patch comes after it;"
		print "each line is a full chain, dependencies included, in an order that"
		print "applies. Regenerated by compat_order in patch2vi.sh. Patches not"
		print "listed here are unconstrained."
		print ""
