...
EXINIT="|sc! \\\\${SEP}|:vis 3${SEP}b0${SEP}%;f> 		A\\\\(SYN_BD, BL1, WH1, YE\\\\)\\\\},
	...
	\\\\{bar_ft, \"\\\\^\\\\(\\\\\\\\\"\\\\.\\\\*\\\\\\\\\"\\\\)\\\\.\\\\*\\\\(\\\\\\\\\\\\\\\\\\\\[\\\\[wrf\\\\]\\\\\\\\\\\\\\\\\\\\]\\\\)\\\\.\\\\*\\\\\$\", A\\\\(AY1 \\\\| SYN_BD, BL, RE\\\\)\\\\},${SEP}??!${DBG:-ya!p\\${SEP}prp\\${SEP}p FAIL conf.c:304\\${SEP}pr${INTR}${QF}}${SEP}${LB}
${SEP}+3${SEP}s/\\\\)\\\\\$/).*\$/${SEP}??!${DBG:-ya!p\\${SEP}prp\\${SEP}p FAIL conf.c:304\\${SEP}pr${INTR}${QF}}${SEP}b1${SEP}%;f> static int vi_status;			/\\\\* permanent status bar \\\\*/
...
${SEP}.,\$;f> 				lbuf_mark\\\\(xb, '\`', orow, ooff\\\\);
			xrow = nrow;
			xoff = noff;${SEP}??!${DBG:-ya!p\\${SEP}prp\\${SEP}p FAIL vi.c:1212\\${SEP}pr${INTR}${QF}}${SEP}${LB}
${SEP}+2a 			if (vi_visual)
				vi_mod |= 1;
${SEP}.,\$;f> 				vi_mod \\\\|= 1;
				break;
...
index 74ffc2d3..63497806 100644
--- a/vi.c
+++ b/vi.c
...
@@ -491,20 +538,23 @@ static void vc_status(int type)
...
@@ -941,6 +991,207 @@ static void vi_shift(int r1, int r2, int dir, int count)
 	free(sb->s);
 }
...
@@ -1210,6 +1461,8 @@ void vi(int init)
...
+				vi_mod |= 1;
 			...
 			char *cmd;
 			term_dec()
@@ -1287,6 +1540,10 @@ void vi(int init)
 				vi_mod |= 1;
 				break;
 			case 'u':
+				if (vi_visual) {
...
+				}
 				...
 				if (vi_arg >= 0 && !lbuf_undo(xb, &xrow, &xoff)) {
 					vi_mod |= 1;
@@ -1337,6 +1594,10 @@ void vi(int init)
 				vi_lncol = 0;
 				vi_mod |= 1;
 				break;
+			case 'U':
...
+				break;
 			case 'v':
 				vi_mod |= 2;
 				k = term_read(0);
@@ -1460,7 +1721,15 @@ void vi(int init)
 					xmpt = 1;
 				break;
 			case 'c':
+				if (vi_visual) {
...
+				}
 				...
 				k = vc_insert(c);
 				insert_done:
@@ -1637,8 +1917,14 @@ void vi(int init)
 					ex_command(cmd)
 					restore(xled)
 					vi_mod |= 1;
-				} else if (k == '~' || k == 'u' || k == 'U')
...
+				}
 			...
 			case 'x':
 				term_push("d ", 2);
@@ -1653,6 +1939,10 @@ void vi(int init)
 				term_push("yy", 2);
 				goto motion;
 			case '~':
+				if (vi_visual) {
...
+				}
 				...
 				goto motion;
 			case 'C':
@@ -1714,6 +2004,13 @@ void vi(int init)
 				vc_status(0);
 				vi_mod |= 1;
 				break;
+			case TK_ESC:
