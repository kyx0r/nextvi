...
2??m 1220reg p OK vi.c:1998:a22sc %? %@2152sc!1q0?
;0fr.,$f> ^				} else if \(k == '\''~'\'' \|\| k == '\''u'\'' \|\| k == '\''U'\''\) \{ \{$3??0?
3??m 1220reg p OK vi.c:1998:a32sc %? %@2152sc!fr 981qfr 980?
%f> 					ex_command\(cmd\)
					restore\(xled\)
					vi_mod \|= 1;4??0?
4??+3m 1220reg p OK vi.c:1998:a42sc %? %@2152sc!1q0?
%f> 					vc_motion\(k\);
				} else if \(k == '\''v'\'' \|\| k == '\''V'\'' \|\| k == '\''b'\''\) \{
					if \(!vi_visual\) \{		/\* fresh selection \*/5??0?
5??-1m 1220reg p OK vi.c:1998:a52sc %? %@2152sc!0?
1;2;3;4;5??!219reg vi.c:1998:r2312sc %? %@2132sc!0?
?0?
%f+ 					}
					vi_visual = vi_visual == k \? 0 : k;
					vi_mod \|= 1;
...
2??m 2220reg p OK vi.c:2007:a22sc %? %@2152sc!1q0?
;0fr.,$f+ ^				}$3??0?
3??m 2220reg p OK vi.c:2007:a32sc %? %@2152sc!fr 981qfr 980?
%f+ 					}
					vi_visual = vi_visual == k \? 0 : k;
					vi_mod \|= 1;4??0?
4??+3m 2220reg p OK vi.c:2007:a42sc %? %@2152sc!1q0?
%f+ 				} else if \(k == '\''K'\''\) \{
					if \(xb_path && xb_path\[0]\)
						lsp_hover\(xb_path, xrow, xoff\);5??0?
5??-1m 2220reg p OK vi.c:2007:a52sc %? %@2152sc!0?
1;2;3;4;5??!219reg vi.c:2007:r2312sc %? %@2132sc!0?
'\''1s/\{ \{/{/??!219reg vi.c:1998:r231:m12sc %? %@2142sc!0?
'\''2d??!219reg vi.c:2007:r231:m22sc %? %@2142sc!p compat 231 applied: src=lsp.sh' > "$P2VIF".231
...
index b5e0f21b..4a0c5f2f 100644
--- a/vi.c
+++ b/vi.c
...
@@ -193,6 +273,7 @@ static void vi_drawrow(int row)
...
+		vi_visual_attrib(s, row);
 		...
 		preserve(int, syn_blockhl, syn_blockhl = -1;)
 		preserve(int, ftidx,)
@@ -212,6 +293,7 @@ static void vi_drawrow(int row)
 		restore(ftidx)
 		return;
 	}
+	vi_visual_attrib(s, row);
...
@@ -495,20 +577,23 @@ static void vc_status(int type)
...
@@ -946,6 +1031,183 @@ static void vi_shift(int r1, int r2, int dir, int count)
 	free(sb->s);
 }
...
@@ -1292,6 +1554,10 @@ void vi(int init)
 				vi_mod |= 1;
 				break;
 			case 'u':
+				if (vi_visual) {
...
+				}
 				...
 				if (vi_arg >= 0 && !lbuf_undo(xb, &xrow, &xoff)) {
 					vi_mod |= 1;
@@ -1342,6 +1608,10 @@ void vi(int init)
 				vi_lncol = 0;
 				vi_mod |= 1;
 				break;
+			case 'U':
...
+				break;
 			case 'v':
 				vi_mod |= 2;
 				k = term_read(0);
@@ -1446,6 +1716,26 @@ void vi(int init)
 				vi_mod |= 1;
 				break;
 			case ':':
...
+				}
 				...
 				do_excmd:
 				if (k && ln[n]) {
@@ -1465,7 +1755,15 @@ void vi(int init)
 					xmpt = 1;
 				break;
 			case 'c':
+				if (vi_visual) {
...
+				}
 				...
 				if (k == 'i') {
 					k = term_read(0);
@@ -1515,6 +1813,10 @@ void vi(int init)
 			case '>':
 			case '<':
 			case TK_CTL('w'):
...
+				}
 				...
 				if (c == 'c')
 					goto insert_done;
@@ -1525,6 +1827,14 @@ void vi(int init)
 			case 'A':
 			case 'o':
 			case 'O':
...
+				}
 				...
 				k = vc_insert(c);
 				insert_done:
@@ -1646,8 +1956,16 @@ void vi(int init)
 					ex_command(cmd)
 					restore(xled)
 					vi_mod |= 1;
-				} else if (k == '~' || k == 'u' || k == 'U')
...
+				}
 			...
 			case 'x':
 				term_push("d ", 2);
@@ -1662,16 +1980,25 @@ void vi(int init)
 				term_push("yy", 2);
 				goto motion;
 			case '~':
+				if (vi_visual) {
...
+					term_push("cc", 2);
 				...
 				icmd_pos--;
 				goto re_motion;
@@ -1737,6 +2064,13 @@ void vi(int init)
 				vc_status(0);
 				vi_mod |= 1;
 				break;
+			case TK_ESC:
...
@@ -1797,6 +2131,8 @@ void vi(int init)
 				}
 			}
 		}
+		if (vi_visual)
