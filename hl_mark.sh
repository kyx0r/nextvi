...
grp 08??-5m 2220reg p OK ex.c:1705:a82sc %? %@2152sc!'\''08??1q0?
m 01;0grp 1%f> 		x->att\[j\+1] = syn_merge\(x->att\[j\+1], conf_hlrev\);
	}
}.*(	if \(xhlr && !p\))
		led_extreg\(\)->ext_func = ext_hlr;
	else if \(!xhlr && p\)9??0?
grp 09??-8m 2220reg p OK ex.c:1705:a92sc %? %@2152sc!'\''00?
1;4;8;9??!219reg ex.c:17052sc %? %@2132sc!0?
?0?
%f+ 	EO\(ts\),
	EO\(td\),
	EO\(order\),
...
index 4d333baf..4066f3b9 100644
--- a/ex.c
+++ b/ex.c
...
@@ -1703,6 +1704,7 @@ EO(pac) EO(pr) EO(ai) EO(err) EO(fr) EO(ish) EO(ic) EO(mpt)
...
@@ -1816,6 +1818,7 @@ static struct excmd {
 	EO(ts),
 	EO(td),
 	EO(order),
+	EO(hlm),
...
index 09100cde..3765b002 100644
--- a/vi.c
+++ b/vi.c
